...
class DOSToUnixPathHelper {
  ...
  private static File constructFile(String prefix) { return new File(prefix); }
  ...
  static void iteratePath(String exe, Helper helper) {
    ...
    if (exe.indexOf(File.separatorChar) >= 0) {
      ...
      String tokenizedPath, path = (String)EnvVars.masterEnvVars.get("PATH");
      ...
      if (path != null) {
        ...
        for (String _dir : Util.tokenize(path.replace("\\", "\\\\"), File.pathSeparator)) {
          ...
          if (checkPrefix(_dir + _dir + File.separator, helper))
            return; 
        } 
