...
public class NoOverlapCategoryAxis extends CategoryAxis {
  public NoOverlapCategoryAxis(String label) { super(label); }
    ...
    if (isTickLabelsVisible()) {
      ...
      for (Object tick1 : ticks) {
        ...
        if (r == null || !r.intersects(area)) {
          Point2D anchorPoint = RectangleAnchor.coordinates(area, position
              .getCategoryAnchor());
          ...
          r.add(r.getMaxX() + r.getWidth() / 2.0D, r.getCenterY());
          r.add(r.getMinX() - r.getWidth() / 2.0D, r.getCenterY());
          r.add(r.getCenterX(), r.getMinY() - r.getHeight() / 2.0D);
          r.add(r.getCenterX(), r.getMaxX() + r.getHeight() / 2.0D);
        } 
        categoryIndex++;
      } 
