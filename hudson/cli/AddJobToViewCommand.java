...
public class AddJobToViewCommand extends CLICommand {
  ...
  public String getShortDescription() { return Messages.AddJobToViewCommand_ShortDescription(); }
  
  protected int run() throws Exception {
    this.view.checkPermission(View.CONFIGURE);
    if (!(this.view instanceof DirectlyModifiableView))
      throw new IllegalStateException("'" + this.view
          .getDisplayName() + "' view can not be modified directly"); 
    for (TopLevelItem job : this.jobs)
      ((DirectlyModifiableView)this.view).add(job); 
    return 0;
  }
}
